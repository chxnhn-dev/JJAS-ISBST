...
Public Class UserRepository
    ...
    Public Function GetUsersPage(request As PagedQueryRequest) As PagedQueryResult
        ...
        Dim normalizedSearch As String = If(safeRequest.SearchText, String.Empty).Trim().ToLowerInvariant()
        Dim likeSearch As String = If(String.IsNullOrWhiteSpace(safeRequest.SearchText), "", "%" & safeRequest.SearchText.Trim() & "%")
        ...
        Dim totalRecords As Integer = Db.ExecuteScalar(Of Integer)(
            countSql,
            New SqlParameter("@search", SqlDbType.NVarChar, 100) With {.Value = normalizedSearch},
            New SqlParameter("@likeSearch", SqlDbType.NVarChar, 150) With {.Value = likeSearch}
        )
        ...
        Dim dataSql As String = "
            ...
            WHERE (@search = ''
                ...
                OR (@search = 'active' AND IsActive = 1)
                OR (@search = 'inactive' AND IsActive = 0))
            ORDER BY IsActive ASC, DateCreated DESC, UserID DESC
