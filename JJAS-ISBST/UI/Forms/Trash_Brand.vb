...
Public Class Trash_Brand
    Private Sub Trash_Brand_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        displayData("")
    ...
    Private Sub displayData(searchtext As String)
        ...
        FROM tbl_Brand WHERE IsActive = 0
        ...
        If DGVtrash.Columns.Contains("BrandID") Then
            DGVtrash.Columns("BrandID").Visible = False
        End If
    ...
    Private Sub btnRestore_Click(sender As Object, e As EventArgs) Handles btnRestore.Click
        ...
        Try
            ...
            If MessageBox.Show("Are you sure you want to restore this row?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Using conn As SqlConnection = DataAccess.GetConnection()
                    conn.Open()
                    Dim sql As String = "UPDATE tbl_Brand SET IsActive = 1, DateCreated=@DateCreated WHERE BrandID = @BrandID"
                    Using cmd As New SqlCommand(sql, conn)
                    ...
                    End Using
                End Using
                LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Restored Brand.")
        ...
        Catch ex As Exception
            MessageBox.Show("An error occurred while restoring brand: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MessageBox.Show("Are you sure you want to permanently delete this row?",
            ...
            Using conn As SqlConnection = DataAccess.GetConnection()
                conn.Open()
                Dim sql As String = "DELETE FROM tbl_Brand WHERE BrandID = @BrandID"
                Using cmd As New SqlCommand(sql, conn)
                    cmd.Parameters.AddWithValue("@BrandID", ID)
                    cmd.ExecuteNonQuery()
                End Using
            End Using
            LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Deleted Brand Permanently.")
