...
Public Class Trash_Category
    Private Sub Trash_Category_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        displayData("")
    ...
    Private Sub displayData(Searchtext As String)
        ...
        FROM tbl_Category WHERE IsActive = 0
        ...
        If DGVtrash.Columns.Contains("CategoryID") Then
            DGVtrash.Columns("CategoryID").Visible = False
        End If
    ...
    Private Sub lblPlaceholder_Click(sender As Object, e As EventArgs) Handles lblPlaceholder.Click
        txtSearch.Focus()
    End Sub
    Private Sub btnRestore_Click(sender As Object, e As EventArgs) Handles btnRefresh.Click
        ...
        Try
            ...
            If MessageBox.Show("Are you sure you want to restore this row?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Using conn As SqlConnection = DataAccess.GetConnection()
                    conn.Open()
                    Dim sql As String = "UPDATE tbl_Category SET IsActive = 1, DateCreated=@DateCreated WHERE CategoryID = @CategoryID"
                    Using cmd As New SqlCommand(sql, conn)
                    ...
                    End Using
                End Using
                LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Restore Category.")
        ...
        Catch ex As Exception
            MessageBox.Show("An error occurred while restoring category: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MessageBox.Show("Are you sure you want to permanently delete this row?",
            ...
            Using conn As SqlConnection = DataAccess.GetConnection()
                conn.Open()
                Dim sql As String = "DELETE FROM tbl_Category WHERE CategoryID = @CategoryID"
                Using cmd As New SqlCommand(sql, conn)
                    cmd.Parameters.AddWithValue("@CategoryID", ID)
                    cmd.ExecuteNonQuery()
                End Using
            End Using
            LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Deleted Category Permanently.")
