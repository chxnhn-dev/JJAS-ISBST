...
Public Class Trash_Color
    Private Sub Trash_Color_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub displayData(searchtext As String)
        ...
        FROM tbl_Color WHERE IsActive = 0
        ...
        If DGVtrash.Columns.Contains("ColorID") Then
            DGVtrash.Columns("ColorID").Visible = False
        End If
    ...
    Private Sub btnRestore_Click(sender As Object, e As EventArgs) Handles btnRestore.Click
        ...
        Try
            ...
            If MessageBox.Show("Are you sure you want to restore this row?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Using conn As SqlConnection = DataAccess.GetConnection()
                    conn.Open()
                    Dim sql As String = "UPDATE tbl_Color SET IsActive = 1, DateCreated=@DateCreated WHERE ColorID = @ColorID"
                    Using cmd As New SqlCommand(sql, conn)
                    ...
                    End Using
                End Using
                LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Restore Color.")
        ...
        Catch ex As Exception
            MessageBox.Show("An error occurred while restoring color: " & ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MessageBox.Show("Are you sure you want to permanently delete this row?",
            ...
            Using conn As SqlConnection = DataAccess.GetConnection()
                conn.Open()
                Dim sql As String = "DELETE FROM tbl_Color WHERE ColorID = @ColorID"
                Using cmd As New SqlCommand(sql, conn)
                    cmd.Parameters.AddWithValue("@ColorID", ID)
                    cmd.ExecuteNonQuery()
                End Using
            End Using
            LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Deleted Color Permanently.")
