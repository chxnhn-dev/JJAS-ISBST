...
Public Class Trash_Size
    Private Sub Trash_Size_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub displayData(Searchtext As String)
        ...
        FROM tbl_Size WHERE IsActive = 0 
        ...
        If DGVtrash.Columns.Contains("SizeID") Then
            DGVtrash.Columns("SizeID").Visible = False
        End If
    ...
    Private Sub btnRestore_Click(sender As Object, e As EventArgs) Handles btnRestore.Click
        ...
        Try
            ...
            If MessageBox.Show("Are you sure you want to restore this row?", "Confirm", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = DialogResult.Yes Then
                Using conn As SqlConnection = DataAccess.GetConnection()
                    conn.Open()
                    Dim sql As String = "UPDATE tbl_Size SET IsActive = 1, DateCreated=@DateCreated WHERE SizeID = @SizeID"
                    Using cmd As New SqlCommand(sql, conn)
                    ...
                    End Using
                End Using
                LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Restored size.")
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MessageBox.Show("Are you sure you want to permanently delete this row?",
            ...
            Using conn As SqlConnection = DataAccess.GetConnection()
                conn.Open()
                Dim sql As String = "DELETE FROM tbl_Size WHERE SizeID = @SizeID"
                Using cmd As New SqlCommand(sql, conn)
                    cmd.Parameters.AddWithValue("@SizeID", ID)
                    cmd.ExecuteNonQuery()
                End Using
            End Using
            LogActivity(CurrentUser.UserID, CurrentUser.FullName, CurrentUser.Username, CurrentUser.Role, "Deleted Size Permanently.")
