...
Public Class FrmLogin
        ...
        Try
            Using conn = DataAccess.GetConnection()
                Using cmd As New SqlCommand("SELECT COUNT(1) FROM Admins", conn)
                    ...
                    If count = 0 Then
                        Using signup As New FrmAdminSignup()
    ...
    Dim username As String = txtUsername1.Text.Trim()
    Dim password As String = txtPassword.Text.Trim()

    Dim auth As New AuthService()
    Dim res As AuthService.LoginResult = auth.FrmLogin(username, password)
    ...
    Else
        MsgBox($"FrmLogin Successful! Welcome {CurrentUser.FullName}", MsgBoxStyle.Information)
    End If

    ' ✅ Navigate
    Select Case CurrentUser.Role
        Case "cashier"
            Dim cashierDashboard As New FrmDashboardCashier()
            cashierDashboard.Show()
        Case "staff"
            Dim staffDashboard As New FrmDashboardStaff()
            staffDashboard.Show()
        Case "admin"
