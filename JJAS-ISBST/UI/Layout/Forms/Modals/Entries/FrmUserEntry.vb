...
Public Class FrmUserEntry
    ...
    Private _editDataLoaded As Boolean
    ...
    Private Sub FrmUserEntry_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        ...
        If IsEditMode AndAlso Not _editDataLoaded Then
            LoadUserForEdit()
        End If
    ...
    Private Sub LoadRoles()
        If cbrole Is Nothing Then Return

        Dim currentRoleText As String = cbrole.Text
        _rolesLoaded = False
        ...
        Dim roleIndex As Integer = cbrole.FindStringExact(currentRoleText)
        cbrole.SelectedIndex = If(roleIndex >= 0, roleIndex, 0)
    ...
    Private Sub ConfigureMode()
        If IsEditMode Then
            Me.Text = "Edit User"
            btnAdd.Text = "Update"
            If lblTitle IsNot Nothing Then lblTitle.Text = "Edit User"
        Else
            Me.Text = "Add User"
            btnAdd.Text = "Save"
            If lblTitle IsNot Nothing Then lblTitle.Text = "Add User"
        End If
    ...
    Private Sub PopulateUserFields(row As DataRow)
        ...
        Dim roleText As String = row("Role").ToString().Trim()
        Dim roleIndex As Integer = cbrole.FindStringExact(roleText)
        cbrole.SelectedIndex = If(roleIndex >= 0, roleIndex, 0)
        If roleIndex < 0 AndAlso Not String.IsNullOrWhiteSpace(roleText) Then
            cbrole.Text = roleText
        End If
